...
Правила:
0. КРИТИЧНО: Если сообщение начинается с коррекции ("нет", "не то", "не так", "неправильно", "ошибка", "нет,") —
   пользователь исправляет предыдущий ответ. Определи интент из ОСНОВНОЙ части сообщения (после слова-коррекции),
   используй историю диалога чтобы понять контекст. Само слово "нет"/"не то" — НЕ название пространства и не объект!
1. Для простых запросов — один интент.
2. "Создай пространство X и загрузи файл" → ["create_namespace", "save_file"]
2a. КРИТИЧНО: "добавь/создай N файлов" БЕЗ прикреплённого файла (пользователь просит сгенерировать или придумать содержимое) → create_file × N (НЕ save_file!).
    Если же [Прикреплён файл] — это всегда save_file, даже если пользователь говорит "добавь файлы" / "загрузи туда файлы"!
...
5. КРИТИЧНО: Если прикреплён файл (флаг [Прикреплён файл]):
   - ЛЮБОЕ упоминание "добавь", "загрузи", "сохрани", "туда", "скинь", "прикреплённые файлы" → ["save_file"]
   - "этот файл", "эти файлы", "файлы" в любом контексте при наличии вложения → save_file
   - "создай пространство X и сохрани этот файл" → ["create_namespace", "save_file"]
   - НЕ используй create_file и НЕ используй save_summary когда есть прикреплённый файл!
...
6b. КРИТИЧНО: Если в сообщении есть ссылка + "о чём", "расскажи", "что это", "суммаризируй" → ["summarize"] (НЕ rag_query!).
    URL + вопрос по содержимому ("найди в этой статье X") → ["rag_query"].
...
7a. КРИТИЧНО: "сохрани в пространство X" / "сохрани в X" / "сохрани туда" — пространство X считается УЖЕ существующим.
    НЕ добавляй create_namespace если пользователь НЕ говорит явно "создай пространство" или "в новое пространство".
    "сохрани эту суммаризацию в пространство Архив" → ["save_summary"] (ОДИН интент, без create_namespace!)
...
"https://example.com о чём эта статья" (+ [В сообщении есть ссылка]) → {"intents": ["summarize"]}
"https://example.com расскажи кратко" (+ [В сообщении есть ссылка]) → {"intents": ["summarize"]}
"https://example.com найди информацию о X" (+ [В сообщении есть ссылка]) → {"intents": ["rag_query"]}
...
"создай пространство Советы и добавь туда 3 файла (содержание придумай сам)" → {"intents": ["create_namespace", "create_file", "create_file", "create_file"]}
"добавь 2 файла в пространство Идеи" (БЕЗ прикреплённого файла) → {"intents": ["create_file", "create_file"]}
...
"добавь туда эти файлы" (+ [Прикреплён файл]) → {"intents": ["save_file"]}
"добавь файлы" (+ [Прикреплён файл]) → {"intents": ["save_file"]}
...
"сохрани эту суммаризацию в пространство Суммаризация" → {"intents": ["save_summary"]}
"сохрани это в новое пространство Архив" → {"intents": ["create_namespace", "save_summary"]}
