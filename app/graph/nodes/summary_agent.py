...
class SummaryAgent:
    ...
    SYSTEM_PROMPT_SUMMARY = (
        ...
        "Твоя задача: составить развёрнутое, структурированное резюме текста. "
        "СТРОГИЕ ПРАВИЛА: "
        ...
        "5. Резюме должно быть подробным: охватывать все ключевые разделы, идеи и факты из текста. "
        "Отвечай на русском языке."
    )

    SYSTEM_PROMPT_CHUNK = (
        "Ты — инструмент суммаризации фрагментов текста. "
        "Перескажи подробно всё содержимое фрагмента между тегами <text> и </text>. "
        "Сохрани все ключевые факты, имена, числа и идеи. "
    ...
    SYSTEM_PROMPT_FINAL = (
        ...
        "Объедини резюме фрагментов в единое развёрнутое резюме документа. "
        "СТРОГО: используй только то, что есть в резюме фрагментов. Ничего от себя. "
        "Убери повторы, но сохрани ВСЕ важные факты, идеи и детали. "
        "Структурируй ответ по разделам если это уместно. "
        "Отвечай на русском языке."
    )
    ...
    async def _stuffing(self, text: str, title: Optional[str] = None) -> SummaryResult:
        ...
        user_message = (
            f"{title_line}"
            f"<text>\n{text}\n</text>\n\n"
            "Составь развёрнутое структурированное резюме документа выше. "
            "Охвати все основные разделы, ключевые идеи, факты, имена и выводы. "
            "Если в тексте есть разделы или темы — отрази их структуру в резюме. "
    ...
    async def _map_reduce(self, text: str, title: Optional[str] = None) -> SummaryResult:
        ...
        async def summarize_chunk(chunk: str, index: int) -> str:
            messages = [
                {"role": "system", "text": self.SYSTEM_PROMPT_CHUNK},
                {
                    ...
                    "text": (
                        f"<text>\n{chunk}\n</text>\n\n"
                        "Подробно перескажи содержимое фрагмента выше: "
                        "все ключевые факты, имена, числа и идеи. "
        ...
        reduce_message = (
            ...
            f"<fragments>\n{combined_summaries}\n</fragments>\n\n"
            "Объедини резюме фрагментов в единое развёрнутое резюме документа. "
            "Сохрани все ключевые факты, идеи, имена и детали. "
            "Структурируй ответ по разделам если это уместно. "
